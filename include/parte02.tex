...
% \begin{frame}{Exercício 9-24}
%     \begin{itemize}[<+->]
%         \item Se \((\vec{u},\vec{v}, \vec{w})\) é base, então \((\vec{u},\vec{v}, \vec{w})\) é LI e
%         \[
%         \alpha \vec{u}+ \beta\vec{v}+\gamma\vec{w}=\vec{0}
%         \]
%         somente quando \(\alpha=\beta=\gamma=0\)
%         \item Como sabemos as medidas angulares entre os vetores \(\vec{u}\) e \(\vec{v}\) e \(\vec{u}\) e \(\vec{w}\), vamos ''multiplicar'' \(\vec{u}\) por ambos os lados da igualdade:
%         \begin{align*}
%             \vec{u}\cdot(\alpha \vec{u}+ \beta\vec{v}+\gamma\vec{w}) &=
%                 \vec{u}\cdot\vec{0} \\
%             \alpha\vec{u}\cdot\vec{u}+
%                 \beta\vec{u}\cdot\vec{v}+\gamma\vec{u}\cdot\vec{w} &=
%                 0 \\
%              \uncover<+-> {\alpha\alert{||\vec{u}||}||\vec{u}||+
%                 \beta\alert{||\vec{u}||}||\vec{v}||\cos{\SI{30}{\degree}}+
%                 \gamma\alert{||\vec{u}||}\vec{w}||\cos{\SI{45}{\degree}} &=0 \\
%               \alpha||\vec{u}||+
%                 \beta||\vec{v}||\frac{\sqrt{3}}{2}+
%                 \gamma||\vec{w}||\frac{\sqrt{2}}{2} &=0 \\}
%         \end{align*}
%     \end{itemize}
...
% \begin{frame}{}
%     \begin{itemize}[<+->]
%         \item Agora vamos ''multiplicar'' \(\vec{v}\):
%         \begin{align*}
%             \vec{v}\cdot(\alpha \vec{u}+ \beta\vec{v}+\gamma\vec{w}) &=
%                 \vec{v}\cdot\vec{0} \\
%             \alpha\vec{v}\cdot\vec{u}+
%                 \beta\vec{v}\cdot\vec{v}+\gamma\vec{v}\cdot\vec{w} &=
%                 0 \\
%              \uncover<+-> {\alpha\alert{||\vec{v}||}||\vec{u}||\cos{\SI{30}{\degree}}+
%                 \beta\alert{||\vec{v}||}||\vec{v}||+
%                 \gamma\alert{||\vec{v}||}\vec{w}||\cos{\SI{90}{\degree}} &=0 \\
%               \alpha||\vec{u}||\frac{\sqrt{3}}{2}+
%                 \beta||\vec{v}|| &=0 \\ }
%         \end{align*}

%         \vspace{-0.5cm} % ''comendo'', na inocência, o espaço após align

%         \item Finalmente, vamos ''multiplicar'' \(\vec{w}\)
%         \begin{align*}
%             \vec{w}\cdot(\alpha \vec{u}+ \beta\vec{v}+\gamma\vec{w}) &=
%                 \vec{w}\cdot\vec{0} \\
%             \alpha\vec{w}\cdot\vec{u}+
%                 \beta\vec{w}\cdot\vec{v}+\gamma\vec{w}\cdot\vec{w} &=
%                 0 \\
%              \uncover<+-> {\alpha\alert{||\vec{w}||}||\vec{u}||\cos{\SI{45}{\degree}}+
%                 \beta\alert{||\vec{w}||}||\vec{v}||\cos{\SI{90}{\degree}}+
%                 \gamma\alert{||\vec{w}||}\vec{w}|| &=0 \\
%               \alpha||\vec{u}||\frac{\sqrt{2}}{2}+
%                 \gamma||\vec{w}|| &=0 \\ }
%         \end{align*}
%     \end{itemize}
...
% \begin{frame}{}
%     \begin{itemize}[<+->]
%         \item Juntamos, então, as 3 equações obtidas:
%         \[
%             \begin{cases}
%              \alpha||\vec{u}||+
%                 \beta||\vec{v}||\frac{\sqrt{3}}{2}+
...
%             \end{cases}
%         \]
%         \item Como \(||\vec{u}||\), \(||\vec{v}||\) e \(||\vec{w}||\) são diferentes de zero, podemos reescrever
%         \[
%             \begin{cases}
%              \alpha'+
%                 \beta'\frac{\sqrt{3}}{2}+
%                 \gamma'\frac{\sqrt{2}}{2} &=0 \\
%             \alpha'\frac{\sqrt{3}}{2}+
%                 \beta' &=0 \\
%             \alpha'\frac{\sqrt{2}}{2}+
%                 \gamma' &=0
%             \end{cases}
%         \]
%         \item Para que \(\alpha'\), \(\beta'\) e \(\gamma'\) possam ser não nulos, segundo a regra de Cramer, o determinante
%         \[
%         \begin{vmatrix}
%         1 & \frac{\sqrt{3}}{2} & \frac{\sqrt{2}}{2} \\
%         \frac{\sqrt{3}}{2} & 1 & 0 \\
%         \frac{\sqrt{2}}{2} & 0 & 1
%         \end{vmatrix}
%         \]
...
% \begin{frame}{}
%     \begin{itemize}[<+->]
%         \item Calculando o determinante, temos que
%         \[
%         \begin{vmatrix}
%         1 & \frac{\sqrt{3}}{2} & \frac{\sqrt{2}}{2} \\
%         \frac{\sqrt{3}}{2} & 1 & 0 \\
%         \frac{\sqrt{2}}{2} & 0 & 1
%         \end{vmatrix}=
%         1+0+0-\left(\frac{\sqrt{2}}{2}\right)^2 -\left(\frac{\sqrt{3}}{2}\right)^2=
%         -\frac{1}{4}
%         \]
%         \item Como consequência, \(\alpha'\), \(\beta'\) e \(\gamma'\) precisam ser nulos e, como consequência, \(\alpha\), \(\beta\) e \(\gamma\) também precisam ser nulos
%         \item Ou seja, \((\vec{u},\vec{v}, \vec{w})\) é LI e, portanto, base \alert{(resposta da letra a)}

%         \item \alert{letra b)} Se \(\vec{u},\vec{v} \text{ e } \vec{w}\) são ortogonais dois a dois, a medida angular entre \(\vec{u}\) e \(\vec{v}\), \(\vec{u}\) e \(\vec{w}\) e \(\vec{v}\) e \(\vec{w}\) é \SI{90}{\degree}
%         \item Como \(\cos{\SI{90}{\degree}}=0\), o determinante no final da letra a, após a modificação, ficaria igual a 1 e, portanto,
%         \(\vec{u},\vec{v} \text{ e } \vec{w}\) são LI
%         \item O mesmo resultado vale para apenas 2 vetores ortogonais não nulos
%     \end{itemize}
